let slice l start stop =
  ...
  | h :: t ->
      if start = 0 && stop > 0 then
        aux t 0 (stop - 1) (h :: acc)
      else if stop = 0 then
        h :: acc
      else
        aux t (start - 1) (stop - 1) acc
  in
  let list_length = List.length l in
  let valid_range = start <= stop || start <= list_length || stop <= list_length in
...
let run () =
  ...
  match slice l start stop with
    | Some l -> print_list l (fun x -> Printf.printf "%d " x);
        print_string "\n"
